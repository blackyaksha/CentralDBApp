...
export default function CurrentFiles() {
  ...
  const [activeFolder, setActiveFolder] = useState<string>('1 PD ONGOING')
...
const fetchAllPages = async (isFirstLoad = false) => {
  try {
    ...
    const newItems = Array.isArray(data.items)
      ? data.items.filter((i: Database) => {
          if (typeof i.FilePath !== 'string') return false
          if (!i.FilePath.startsWith(TARGET_FOLDER)) return false

          const relative = i.FilePath.slice(TARGET_FOLDER.length)
      ...
      <div className={'gallery-grid' + (lowerQuery ? ' horizontal' : '')}>
        ...
        {false && (
          <article
            ...
            onClick={() => setActiveFolder('1 PD ONGOING')}
          >
